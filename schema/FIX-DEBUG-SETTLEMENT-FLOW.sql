...
-- 2. Fixed get_user_pending_settlements function
...
-- 3. Simple test function to verify settlement creation
...
BEGIN
  ...
  SELECT COUNT(*) INTO v_pending_count FROM settlements WHERE status = 'pending';
  
  v_result := 'Total settlements: ' || v_settlement_count || ', Pending: ' || v_pending_count;
...
-- 4. Simple function to check if a specific user has pending settlements
CREATE OR REPLACE FUNCTION check_user_pending_settlements(p_user_id UUID)
RETURNS TEXT AS $$
DECLARE
  v_count INTEGER;
  v_result TEXT;
BEGIN
  -- Count pending settlements for this user as receiver
  SELECT COUNT(*) INTO v_count
  FROM settlements 
  WHERE status = 'pending' AND receiver_id = p_user_id;
  
  v_result := 'User ' || p_user_id || ' has ' || v_count || ' pending settlements as receiver';
...
-- Grant permissions
GRANT EXECUTE ON FUNCTION check_user_pending_settlements(UUID) TO authenticated;

-- 5. Enhanced submit_settlement with better logging
...
BEGIN
  ...
  -- Get the group creator (receiver)
  ...
  IF v_payer_id = v_receiver_id THEN
    RAISE EXCEPTION 'User % cannot submit settlement to themselves', v_payer_id;
  END IF;
  ...
  RAISE NOTICE 'Settlement created with ID: %', v_settlement_id;
  
  -- Verify the settlement was created correctly
...
SELECT 'Fixed debug functions created successfully!' as status;